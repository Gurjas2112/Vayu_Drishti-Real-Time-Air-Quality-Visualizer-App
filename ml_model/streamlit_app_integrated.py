...
def generate_realistic_data(location, hours=24):
    ...
    base = all_base_aqi.get(location, 80)
    
    # Use time-dependent seed for different patterns at different times
    np.random.seed(int(current_time.timestamp()) % 10000)
    
    # Progressive trend varies by forecast length
    trend_direction = np.random.choice([-1, 1])
    trend_strength = 0.15 if hours <= 24 else (0.25 if hours <= 48 else 0.35)
    
    # Weather system influence (36-hour cycle for realistic patterns)
    weather_amplitude = 15 * (hours / 24)
    ...
    for i, time in enumerate(times):
        ...
        # Progressive trend with sinusoidal variation
        progressive_trend = trend_direction * trend_strength * i * (1 + np.sin(i / 12))
        
        # Weather system influence (simulates passing weather fronts)
        weather_influence = weather_amplitude * np.sin(2 * np.pi * i / 36)
        
        # Meteorological impact on dispersion with hour-by-hour variation
        hour_wind = 2 + 3 * (1 - np.exp(-((hour - 14)**2) / 20)) + np.random.uniform(-1, 1)
        hour_wind = max(0.5, hour_wind)
        wind_dispersion_factor = 1 - (hour_wind / 15)  # Higher wind = lower pollution
        
        # Random variation increases with forecast horizon
        noise_std = 5 + (i * 0.3) + (hours / 24 * 2)  # Uncertainty increases with time
        noise = np.random.normal(0, noise_std)
        
        # Random pollution events (5% chance per hour)
        pollution_spike = np.random.uniform(10, 30) if np.random.random() < 0.05 else 0
        
        # Day-to-day baseline variation for multi-day forecasts
        if i > 0 and i % 24 == 0:
            base += np.random.uniform(-10, 15)
        
        aqi = max(20, base * season_factor + 
                  (diurnal * weekly_factor * wind_dispersion_factor) + 
                  progressive_trend + weather_influence + noise + pollution_spike)
...
def predict_aqi_rf(features_dict, interpreter, scaler, feature_names_list):
    ...
    # Sanity checks and clamping to realistic AQI range
    # Standard AQI scale: 0 - 500 (values >500 are considered beyond index and treated as 'Hazardous')
    # Some models can produce erroneous large outputs due to numerical issues; clamp defensively.
    if pred_value < 0:
        pred_value = 0.0
    if pred_value > 500:
        # Cap at 500 to avoid unrealistic reported values
        pred_value = 500.0
